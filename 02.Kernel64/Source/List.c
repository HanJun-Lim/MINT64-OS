...
void* kRemoveList(LIST* pstList, QWORD qwID)		// core function for other removal functions
{
	LISTLINK* pstLink;
	...
	{
		// Search item that ID matches
		if(pstLink->qwID == qwID)
		{
			// case of only one item : initialize List
		...
		pstPreviousLink = pstLink;		// save previous data
	}
